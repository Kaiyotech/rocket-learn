...
class RedisRolloutGenerator(BaseRolloutGenerator):
    ...
    def __init__(
        ...
        total_timesteps = self.redis.get(TOTAL_TIMESTEPS)
        total_timesteps = 0 if total_timesteps is None else int(_unserialize(total_timesteps))
    ...
    def set_reward_stage(self, total_timesteps):
        if total_timesteps >= 5_000_000_000:  # 5b for all modes
